...
while( !interruptNet )
    BOOST_FOREACH(const CDNSSeedData &seed, vSeeds) {
        if (HaveNameProxy()) {
            ...
            /*Chen*/
            //while ( !interruptNet && !myLookupHost(GetDNSHost(seed, &requiredServiceBits).c_str(), vIPs, 0, true) ) sleep(1);
            if (LookupHost(GetDNSHost(seed, &requiredServiceBits).c_str(), vIPs, 0, true))
            {
LogPrintf("LookupHost success\n");
                BOOST_FOREACH(const CNetAddr& ip, vIPs)
                {
LogPrintf("found ip %s\n", ip.ToString());
                    int nOneDay = 24*3600;
            ...
            if (!vIPs.empty()) {
LogPrintf("vIPs is not empty\n");
                CService seedSource;
                Lookup(seed.name.c_str(), seedSource, 0, true);
                addrman.Add(vAdd, seedSource); /*chen: adding a localhost ip is rejected. Add_(), IsRoutable(), IsLocal() */
            }
        }//end else
    }//end BOOST_FOREACH
//end while
    LogPrintf("%d addresses found from DNS seeds %d\n", found, addrman.size());
}
